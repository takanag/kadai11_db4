...
<body>
    ...
    <main>
        <div class="table-container">
            ...
            //2. データ登録SQL作成
            $stmt = $pdo->prepare('SELECT * FROM user_master');
            $status = $stmt->execute();
            ...
            echo "<tr><th>ユーザーID</th><th>ユーザー名</th><th>Email</th><th>ポイント</th><th>管理者</th><th>新規登録日時</th><th>更新日時</th><th>削除日時</th><th>更新</th><th>削除</th></tr>";
            ...
            foreach ($results as $row) {
                ...
                echo "<td>" . h($row['point']) . "</td>";
                echo "<td>" . h($row['admin']) . "</td>";
                echo "<td>" . h($row['created']) . "</td>";
                echo "<td>" . h($row['updated']) . "</td>";
                echo "<td>" . h($row['deleted']) . "</td>";

                // 更新と削除のリンクを一列に正しく配置
                echo "<td><a href='bandai_user_detail.php?user_id=" . $row['user_id'] . "'>";
                echo "<img src='" . $img_update . "' alt='update' width='30' height='30'></a></td>";
                echo "<td><a href='bandai_user_delete.php?user_id=" . $row['user_id'] . "' onclick='return confirm(\"削除してよろしいですか？\");'>";
